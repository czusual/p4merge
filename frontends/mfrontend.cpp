...
#include "toP4/toP4.h"
#include "toP4/toRenameP4.h" //Zoe
#include "typeChecking/typeChecker.h"
#include "uniqueNames.h"
#include "mergeReNames.h" //Zoe
...
const IR::P4Program *MergeFrontEnd::run(const CompilerOptions &options, const IR::P4Program* program,
                                   const char *p,bool skipSideEffectOrdering) {
    if (program == nullptr)
        return nullptr;
    ...
    /*
    cstring suffix = cstring("-") + cstring("Zoe");
    cstring filename = options.file;
    if (filename == "-")
        filename = "tmp.p4";

    cstring fileName = makeFileName(options.dumpFolder, filename, suffix);
    */
    ...
    PassManager passes = {
        ...
        new MergeReNames(&refMap,p),                // Rename each node for Mergeing process //Zoe 
        new MUniqueParameters(&refMap, &typeMap,p), // Rename each node for Mergeing process //Zoe 
        new FrontEndLast(),
    };

    passes.setName("MergeFrontEnd");
    ...
    if (stream != nullptr) {
        ...
        P4::ToRenameP4 toRenameP4(stream, Log::verbose(),options.file); //now postFix hardcode,easy to improve by using global var
        result->apply(toRenameP4);
    }
    */
