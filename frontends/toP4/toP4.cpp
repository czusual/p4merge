...
bool ToP4::preorder(const IR::P4Table* c) {
    ...
    builder.append(c->name);
//Zoe
    //mergeRenameFile.open("MergeRenameList.txt",std::ios::app);
    mergeRenameFile<<c->name.originalName<<" "<<c->name.name<<std::endl;
    //mergeRenameFile.close();           
    std::cout<<c->name.originalName<<" "<<c->name.name<<std::endl;
//
    builder.spc();
    builder.blockStart();
    setVecSep("\n", "\n");
