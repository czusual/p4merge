...
#include <sstream>
#include <string>
#include "toRenameP4.h"
#include "frontends/common/options.h"

namespace P4 {

Visitor::profile_t ToRenameP4::init_apply(const IR::Node* node) {
    LOG4("Program dump:" << std::endl << dumpToString(node));
    return Inspector::init_apply(node);
}

void ToRenameP4::end_apply(const IR::Node*) {
...
bool ToRenameP4::isSystemFile(cstring file) {
    if (file.startsWith(p4includePath)) return true;
    return false;
}

cstring ToRenameP4::ifSystemFile(const IR::Node* node) {
...
unsigned ToRenameP4::curDepth() const {
    unsigned result = 0;
    auto ctx = getContext();
    while (ctx != nullptr) {
        ctx = ctx->parent;
...
void ToRenameP4::dump(unsigned depth, const IR::Node* node, unsigned adjDepth) {
    if (!showIR)
        return;
    if (node == nullptr)
        node = getOriginal();
...
bool ToRenameP4::preorder(const IR::P4Program* program) {
...
bool ToRenameP4::preorder(const IR::Type_Bits* t) {
    builder.appendFormat(t->toString());
    return false;
}

bool ToRenameP4::preorder(const IR::Type_InfInt* t) {
    builder.appendFormat(t->toString());
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Var* t) {
    builder.append(t->name+postFix);
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Unknown*) {
    BUG("Cannot emit code for an unknown type");
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Dontcare*) {
    builder.append("_");
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Void*) {
    builder.append("void");
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Name* t) {
    visit(t->path);
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Stack* t) {
    dump(2);
    visit(t->elementType);
    builder.append("[");
    visit(t->size);
...
bool ToRenameP4::preorder(const IR::Type_Specialized* t) {
    dump(3);
    visit(t->baseType);
    builder.append("<");
    setVecSep(", ");
...
bool ToRenameP4::preorder(const IR::Type_Typedef* t) {
    dump(2);
    visit(t->annotations);
...
bool ToRenameP4::preorder(const IR::Type_Tuple* t) {
    dump(3);
...
bool ToRenameP4::preorder(const IR::Type_ValueSet* t) {
    dump(3);
...
bool ToRenameP4::preorder(const IR::Type_Enum* t) {
    dump(1);
    builder.append("enum ");
    builder.append(t->name+postFix);
    builder.spc();
...
bool ToRenameP4::preorder(const IR::TypeParameters* t) {
    if (!t->empty()) {
        builder.append("<");
        bool first = true;
...
bool ToRenameP4::preorder(const IR::Method* m) {
    dump(1);
    visit(m->annotations);
...
bool ToRenameP4::preorder(const IR::Function* function) {
    dump(1);
    ...
    builder.append(function->name+postFix);
    visit(t->typeParameters);
    visit(t->parameters);
    builder.spc();
    visit(function->body);
...
bool ToRenameP4::preorder(const IR::Type_Extern* t) {
    dump(2);
    visit(t->annotations);
    builder.append("extern ");
    builder.append(t->name+postFix);
...
bool ToRenameP4::preorder(const IR::Type_Boolean*) {
    builder.append("bool");
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Varbits* t) {
    builder.appendFormat("varbit<%d>", t->size);
    return false;
}

bool ToRenameP4::preorder(const IR::Type_Package* package) {
    dump(2);
    builder.emitIndent();
    visit(package->annotations);
    builder.append("package ");
    builder.append(package->name+postFix);
...
bool ToRenameP4::process(const IR::Type_StructLike* t, const char* name) {
    dump(2);
    builder.emitIndent();
    visit(t->annotations);
...
bool ToRenameP4::preorder(const IR::Type_Parser* t) {
    dump(2);
    builder.emitIndent();
    visit(t->annotations);
...
bool ToRenameP4::preorder(const IR::Type_Control* t) {
    dump(2);
    builder.emitIndent();
    visit(t->annotations);
...
bool ToRenameP4::preorder(const IR::Constant* c) {
...
bool ToRenameP4::preorder(const IR::BoolLiteral* b) {
    builder.append(b->toString());
    return false;
}

bool ToRenameP4::preorder(const IR::StringLiteral* s) {
    builder.append(s->toString());
    return false;
}

bool ToRenameP4::preorder(const IR::Declaration_Constant* cst) {
    dump(2);
    ...
    visit(type);
    builder.spc();
    builder.append(cst->name+postFix);
    builder.append(" = ");
...
bool ToRenameP4::preorder(const IR::Declaration_Instance* i) {
    dump(3);
    visit(i->annotations);
...
bool ToRenameP4::preorder(const IR::Declaration_Variable* v) {
    dump(1);
    visit(v->annotations);
...
bool ToRenameP4::preorder(const IR::Type_Error* d) {
    dump(1);
    bool first = true;
...
bool ToRenameP4::preorder(const IR::Declaration_MatchKind* d) {
    dump(1);
...
#define VECTOR_VISIT(V, T)                                \
    bool ToRenameP4::preorder(const IR:: V <IR::T> *v) {        \
...
bool ToRenameP4::preorder(const IR::Slice* slice) {
...
bool ToRenameP4::preorder(const IR::DefaultExpression*) {
...
bool ToRenameP4::preorder(const IR::This*) {
    builder.append("this");
    return false;
}

bool ToRenameP4::preorder(const IR::PathExpression* p) {
    visit(p->path);
    return false;
}

bool ToRenameP4::preorder(const IR::TypeNameExpression* e) {
    visit(e->typeName);
    return false;
}

bool ToRenameP4::preorder(const IR::ConstructorCallExpression* e) {
...
bool ToRenameP4::preorder(const IR::Member* e) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::SelectCase* e) {
    dump(2);
    int prec = expressionPrecedence;
    expressionPrecedence = DBPrint::Prec_Low;
    setListTerm("(", ")");
...
bool ToRenameP4::preorder(const IR::SelectExpression* e) {
...
bool ToRenameP4::preorder(const IR::ListExpression* e) {
    cstring start, end;
...
bool ToRenameP4::preorder(const IR::MethodCallExpression* e) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::Operation_Binary* b) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::Mux* b) {
    int prec = expressionPrecedence;
    bool useParens = prec >= b->getPrecedence();
    if (useParens)
        builder.append("(");
...
bool ToRenameP4::preorder(const IR::Operation_Unary* u) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::ArrayIndex* a) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::Cast* c) {
    int prec = expressionPrecedence;
...
bool ToRenameP4::preorder(const IR::AssignmentStatement* a) {
    dump(2);
    visit(a->left);
    builder.append(" = ");
    visit(a->right);
...
bool ToRenameP4::preorder(const IR::BlockStatement* s) {
    dump(1);
    visit(s->annotations);
    builder.blockStart();
    setVecSep("\n", "\n");
...
bool ToRenameP4::preorder(const IR::ExitStatement*) {
    dump(1);
    builder.append("exit");
    builder.endOfStatement();
    return false;
...
bool ToRenameP4::preorder(const IR::ReturnStatement* statement) {
    dump(2);
...
bool ToRenameP4::preorder(const IR::EmptyStatement*) {
    dump(1);
    builder.endOfStatement();
    return false;
}

bool ToRenameP4::preorder(const IR::IfStatement* s) {
    dump(1);
    builder.append("if (");
    visit(s->condition);
    builder.append(") ");
...
bool ToRenameP4::preorder(const IR::MethodCallStatement* s) {
    dump(3);
    visit(s->methodCall);
    builder.endOfStatement();
    return false;
...
bool ToRenameP4::preorder(const IR::SwitchCase* s) {
    visit(s->label);
    builder.append(": ");
    visit(s->statement);
    return false;
...
bool ToRenameP4::preorder(const IR::SwitchStatement* s) {
    dump(4);
...
bool ToRenameP4::preorder(const IR::Annotation * a) {
    builder.append("@");
    builder.append(a->name+postFix);
    if (!a->expr.empty()) {
        builder.append("(");
        setVecSep(", ");
        preorder(&a->expr);
...
bool ToRenameP4::preorder(const IR::Parameter* p) {
    dump(2);
    visit(p->annotations);
    switch (p->direction) {
    ...
    visit(p->type);
    builder.spc();
    builder.append(p->name+postFix);
    return false;
}

bool ToRenameP4::preorder(const IR::P4Control * c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool ToRenameP4::preorder(const IR::ParameterList* p) {
    builder.append("(");
    bool first = true;
...
bool ToRenameP4::preorder(const IR::P4Action * c) {
    dump(2);
    visit(c->annotations);
    builder.append("action ");
    builder.append(c->name+postFix);
    visit(c->parameters);
    builder.spc();
    visit(c->body);
    return false;
...
bool ToRenameP4::preorder(const IR::ParserState* s) {
    dump(1);
    if (s->isBuiltin()) return false;

    visit(s->annotations);
    builder.append("state ");
    builder.append(s->name+postFix);
    builder.spc();
    builder.blockStart();
    setVecSep("\n", "\n");
    preorder(&s->components);
...
bool ToRenameP4::preorder(const IR::P4Parser * c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool ToRenameP4::preorder(const IR::ExpressionValue* v) {
    dump(2);
    visit(v->expression);
    builder.endOfStatement();
    return false;
...
bool ToRenameP4::preorder(const IR::ActionListElement* ale) {
    dump(3);
...
bool ToRenameP4::preorder(const IR::ActionList* v) {
    dump(2);
    builder.blockStart();
    setVecSep(";\n", ";\n");
...
bool ToRenameP4::preorder(const IR::Key* v) {
    dump(2);
    builder.blockStart();
...
bool ToRenameP4::preorder(const IR::Property* p) {
    dump(1);
    visit(p->annotations);
    if (p->isConstant)
        builder.append("const ");
...
bool ToRenameP4::preorder(const IR::TableProperties* t) {
...
bool ToRenameP4::preorder(const IR::EntriesList *l) {
    dump(1);
    builder.append("{");
    builder.newline();
    builder.increaseIndent();
...
bool ToRenameP4::preorder(const IR::Entry *e) {
    dump(2);
    builder.emitIndent();
...
bool ToRenameP4::preorder(const IR::P4Table* c) {
    dump(2);
    visit(c->annotations);
...
bool ToRenameP4::preorder(const IR::Path* p) {
